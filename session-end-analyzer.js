(function(){
'use strict';
/* Chinese Structure Lab — Session End Analyzer v1
   Learning-time rule: collect raw events only. No underground analysis.
   End-of-session rule: lazy-load underground modules, analyze once, persist results
   for the next learning session, then allow navigation.
*/
var VERSION=1,loading=null;
var HEAVY=[
...
function analyze(meta){meta=meta||{};var started=Date.now();
 return loadAll().then(function(){
   ...
   var result={version:VERSION,analyzedAt:now(),durationMs:Date.now()-started,meta:meta,deep:deep?{status:deep.floors&&deep.floors[49]&&deep.floors[49].status||null}:null,integration:integration?{action:integration.decision&&integration.decision.action||null}:null,policy:{sessionEndOnly:true,noRealtimeAnalysis:true,lazyLoaded:true}};
   ...
   return result
 }).catch(function(err){var result={version:VERSION,analyzedAt:now(),durationMs:Date.now()-started,meta:meta,error:String(err&&err.message||err),policy:{sessionEndOnly:true,noRealtimeAnalysis:true,lazyLoaded:true,failOpen:true}};try{window.dispatchEvent(new CustomEvent('csl:session-analysis-error',{detail:result}))}catch(e){}return result})
}
window.CSLSessionEndAnalyzer={version:VERSION,analyze:analyze,loadAll:loadAll,policy:{sessionEndOnly:true,noRealtimeAnalysis:true,lazyLoadUnderground:true}};
})();
