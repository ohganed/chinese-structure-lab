...
var VERSION=1,EXT='undergroundGovernorV1';
...
function decide(){var confidence=safe(function(){return window.CSLEvidenceConfidence&&CSLEvidenceConfidence.get()}),budget=safe(function(){return window.CSLInterventionBudget&&CSLInterventionBudget.status()}),agency=safe(function(){return window.CSLLearnerAgency&&CSLLearnerAgency.get()}),rhythm=safe(function(){return window.CSLLearningRhythm&&CSLLearningRhythm.get()}),load=safe(function(){return window.CSLCognitiveLoadGuard&&CSLCognitiveLoadGuard.get()}),opp=safe(function(){return window.CSLOpportunityMatching&&CSLOpportunityMatching.get()}),audit=safe(function(){return window.CSLQualityAuditor&&CSLQualityAuditor.get()});var reasons=[],action='DO_NOTHING',candidate=null;
 ...
 var result={version:VERSION,decidedAt:now(),action:action,candidate:candidate,reasons:reasons,context:{confidence:confidence&&confidence.confidence||null,permission:confidence&&confidence.permission||'unknown',budgetRemaining:budget&&budget.budget?budget.budget.remaining:null,rhythm:rhythm&&rhythm.mode||'unknown',load:load&&load.band||'unknown',audit:audit&&audit.status||'unknown'},agencySummary:agency&&agency.guidance||null,policy:{doNothingIsFirstClass:true,learnerAgencyOutranksInference:true,accessibilityOutranksAdaptation:true,noDirectUIControl:true,noHistoryMutation:true,reversibleActionsOnly:true}};var s=S();if(s&&s.load&&s.save){var p=s.load();p.extensions=p.extensions||{};p.extensions[EXT]=result;s.save(p)}return result}
