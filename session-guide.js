...
function endLearning(){var btn=document.getElementById('cslTimeStop');btn.disabled=true;btn.textContent='学習を整理中…';finish('stop');var meta={minutes:s.minutes,elapsedSeconds:Math.round((Date.now()-start)/1000),moments:s.moments,targetMoments:target,density:density,page:path};try{window.dispatchEvent(new CustomEvent('csl:session-ended',{detail:meta}))}catch(e){}var p=window.CSLSessionEndAnalyzer&&CSLSessionEndAnalyzer.analyze?CSLSessionEndAnalyzer.analyze(meta):Promise.resolve(null);Promise.resolve(p).then(function(){location.href='./index.html'}).catch(function(){location.href='./index.html'})}
document.getElementById('cslTimeStop').onclick=endLearning;document.getElementById('cslTimeMore').onclick=function(){finish('continue');end.classList.remove('on');guide.remove();document.body.classList.remove('csl-session-'+density)};
