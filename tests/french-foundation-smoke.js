...
for(const meta of index.lessons){const lesson=JSON.parse(fs.readFileSync(`french/${meta.path.replace(/^\.\//,'')}`,'utf8'));for(const key of ['id','level','theme','sentence','translation_ja','audio','sound_groups','sound_phenomena','words','expressions','transforms','meaning_chunks','sentence_structure'])assert(Object.prototype.hasOwnProperty.call(lesson,key),`${meta.id}: missing lesson key ${key}`);assert(lesson.schema_version,`${meta.id}: schema_version is required`);assert(lesson.words.length>=4,`${meta.id}: meaningful word structure required`);const ids=new Set(lesson.words.map(w=>w.id));assert(ids.size===lesson.words.length,`${meta.id}: every word needs a unique id`);for(const expression of lesson.expressions){assert(expression.id&&expression.surface&&expression.meaning_ja,`${meta.id}: expression needs id/surface/meaning`);for(const wordId of expression.word_ids||[])assert(ids.has(wordId),`${meta.id}: expression references unknown word id ${wordId}`);}for(const transform of lesson.transforms)assert(transform.id&&transform.target&&Array.isArray(transform.changes),`${meta.id}: transform needs id/target/changes`);}

const lexemes=JSON.parse(fs.readFileSync(lexemePath,'utf8'));
assert(lexemes.schema_version==='1.0','shared lexeme registry needs schema version');
assert(Array.isArray(lexemes.lexemes)&&lexemes.lexemes.length>=10,'shared lexeme registry needs useful coverage');
...
console.log('French Structure Lab shared lexeme + verified expression matching smoke: OK');
