...
assert(lexicon.schema_version==='1.0','My Text lexicon needs schema version');
assert(lexicon.source==='built-in-lessons','My Text lexicon must identify verified built-in source');
assert(Array.isArray(lexicon.entries)&&lexicon.entries.length>=20,'core lexicon needs useful verified coverage');
for(const entry of lexicon.entries){assert(entry.form&&entry.meaning_ja&&entry.confidence==='verified','lexicon entries must be verified and explicit');}
...
assert(myText.includes("const LEXICON_PATH='./data/lexicon/a1-core.json'"),'My Text should lazy-load a local verified lexicon');
assert(myText.includes('tokenizeFrench'),'My Text needs lightweight local tokenization');
assert(myText.includes('ensureLexicon'),'My Text must match only against explicit local lexicon data');
...
console.log('French Structure Lab My Text observation + safe lexicon smoke: OK');
