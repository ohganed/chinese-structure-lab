(function(){
'use strict';
/* Underground Integration Bus v2
...
var VERSION=2,EXT='undergroundIntegrationV1',timer=null,lastFingerprint='';
...
function run(reason){var snap=normalized(),decision=read('CSLUndergroundGovernor','decide')||{action:'DO_NOTHING',reasons:['governor-unavailable']};var agency=applyAgency(decision,snap.agency,snap.presentation);snap.presentation=agency.presentation;var fingerprint=JSON.stringify([decision.action,decision.candidate&&decision.candidate.id,decision.reasons,snap.presentation,snap.confidence&&snap.confidence.permission]);var budgetResult={consumed:false,reason:'duplicate-decision'};if(fingerprint!==lastFingerprint){budgetResult=consumeBudget(decision);lastFingerprint=fingerprint;if(decision.action==='ALLOW_GENTLE_OPPORTUNITY'&&!budgetResult.consumed){decision={version:VERSION,decidedAt:now(),action:'DO_NOTHING',candidate:null,reasons:['budget-denied',budgetResult.reason],policy:{budgetMustAuthorizeIntervention:true}}}}
