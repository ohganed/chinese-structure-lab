...
function patch(x){x=cp(x);var m;
 ...
 if(/^csl-a1x-permission-/.test(x.id)&&/^在这里.+没问题吗？$/.test(x.zh)){
  var action=x.zh.slice(3,-5),apy=x.py.replace(/^zài zhèlǐ /,'').replace(/ méi wèntí ma$/,'');
  x.zh='请问，在这里'+action+'可以吗？';x.py='qǐngwèn, zài zhèlǐ '+apy+' kěyǐ ma';
  x.words=[W('请问','qǐngwèn','すみません／お尋ねします'),W('在这里','zài zhèlǐ','ここで'),W(action,apy,'この動作'),W('可以吗','kěyǐ ma','いいですか')];x.auditNote='simpler A1 permission request';
 }
