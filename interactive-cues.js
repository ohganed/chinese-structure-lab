...
function mark(root){installStyle();var els=(root||document).querySelectorAll('button,a[href],[role="button"],summary,[onclick]');els.forEach(function(el){var cue=actionable(el)&&visible(el)&&!el.classList.contains('csl-no-cue'),audio=cue&&isAudio(el),expand=cue&&!audio&&expands(el);el.classList.toggle('csl-cue',cue);el.classList.toggle('csl-cue-audio',audio);el.classList.toggle('csl-cue-expand',expand);if(cue&&!el.hasAttribute('aria-label')&&!(el.textContent||'').trim())el.setAttribute('aria-label','Action')})}
