...
/* Interactive cues v2.
   ...
   Other actions use no chevron, so the UI never promises a hidden panel that does not exist. */
var STYLE='csl-interactive-cue-style';
function installStyle(){var old=document.getElementById(STYLE);if(old)old.remove();var s=document.createElement('style');s.id=STYLE;s.textContent=`
...
function mark(root){installStyle();var els=(root||document).querySelectorAll('button,a[href],[role="button"],summary,[onclick]');els.forEach(function(el){el.classList.remove('csl-cue-expand','csl-cue-audio');if(!actionable(el)||!visible(el)||el.classList.contains('csl-no-cue')){el.classList.remove('csl-cue');return}el.classList.add('csl-cue');if(isAudio(el))el.classList.add('csl-cue-audio');else if(expands(el))el.classList.add('csl-cue-expand');if(!el.hasAttribute('aria-label')&&!(el.textContent||'').trim())el.setAttribute('aria-label','Action')})}
function boot(){mark(document);var timer=null;new MutationObserver(function(){clearTimeout(timer);timer=setTimeout(function(){mark(document)},90)}).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['style','class','hidden','aria-expanded','aria-controls']})}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
window.CSLInteractiveCues={refresh:function(){mark(document)},version:2};
})();