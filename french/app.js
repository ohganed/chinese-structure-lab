const state={lesson:null,mode:'listen',selectedWord:null,drawerOpen:false};
...
function bindStaticEvents(){
  ...
  els.workspace.addEventListener('click',(event)=>{
    const button=event.target.closest('[data-word-index]');if(!button||!state.lesson)return;
    const index=Number(button.dataset.wordIndex);openWord(index);
  });
...
function renderLesson(){
  els.sentence.textContent=state.lesson.sentence;
  els.translation.textContent=state.lesson.translation_ja;
}
...
function renderMode(){
  ...
  switch(state.mode){
    case'listen':
      ...
      $('#showMeaning').addEventListener('click',()=>{els.translation.hidden=false;});
      break;
    case'words':
      els.workspace.innerHTML=`<div class="word-list">${lesson.words.map((w,i)=>`<button class="word-chip" type="button" data-word-index="${i}" aria-expanded="${state.selectedWord===i&&state.drawerOpen}">${escapeHtml(w.surface)}</button>`).join('')}</div><p class="hint">1回押すと発音し、同じ操作で詳細を安全に開きます。別の語を押すと内容だけ切り替わります。</p>`;
      break;
    case'chunks':
      els.translation.hidden=false;
      els.workspace.innerHTML=`<div class="chunk-list">${lesson.meaning_chunks.map(c=>`<div class="chunk"><b>${escapeHtml(c.text)}</b><span>${escapeHtml(c.meaning_ja)}</span></div>`).join('')}</div>`;
      break;
    case'structure':
      els.translation.hidden=false;
      els.workspace.innerHTML=`<div class="structure-list">${Object.entries(lesson.sentence_structure).map(([k,v])=>`<div class="structure-piece"><b>${escapeHtml(v)}</b><span>${escapeHtml(structureLabel(k))}</span></div>`).join('')}</div>`;
      break;
    case'sound':
      els.workspace.innerHTML=lesson.sound_groups.map(g=>`<div class="sound-group"><div class="fr">${escapeHtml(g.text)}</div><div class="ipa">/${escapeHtml(g.ipa)}/</div></div>`).join('');
      break;
...
function openWord(index){
  ...
  state.selectedWord=index;state.drawerOpen=true;
  speak(word.surface,.9);
  els.detailSurface.textContent=word.surface;
...
function closeWord(){
  ...
  state.drawerOpen=false;els.drawer.classList.remove('open');els.drawer.setAttribute('aria-hidden','true');els.scrim.hidden=true;updateWordAria();
}

function updateWordAria(){
  $$('[data-word-index]').forEach(button=>button.setAttribute('aria-expanded',String(state.drawerOpen&&Number(button.dataset.wordIndex)===state.selectedWord)));
}

function speak(text,rate=1){
  if(!('speechSynthesis'in window))return;
  speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(text);utterance.lang='fr-FR';utterance.rate=Math.max(.65,Math.min(1.05,rate));speechSynthesis.speak(utterance);
}

function structureLabel(key){return({subject:'話し手・主語',verb:'文の中心・動詞',destination:'目的地',means:'手段'})[key]||key;}
function escapeHtml(value=''){return String(value).replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));}

window.FSL_DEBUG={state,setMode,openWord,closeWord};
