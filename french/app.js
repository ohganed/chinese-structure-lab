...
const els={sentence:$('#sentence'),translation:$('#translation'),workspace:$('#workspace'),drawer:$('#wordDrawer'),detailSurface:$('#detailSurface'),detailBody:$('#detailBody'),scrim:$('#scrim'),picker:$('#lessonPicker'),prev:$('#prevLesson'),next:$('#nextLesson')};
...
async function init(){
  ...
  try{
    ...
    const index=await indexRes.json();state.lessonIndex=Array.isArray(index.lessons)?index.lessons:[];
    if(!state.lessonIndex.length)throw new Error('教材索引が空です。');
    renderLessonPicker();await loadLesson(resolveLessonId());
...
function bindStaticEvents(){
  ...
  els.picker.addEventListener('change',()=>goToLesson(els.picker.value));
  els.prev.addEventListener('click',()=>moveLesson(-1));els.next.addEventListener('click',()=>moveLesson(1));
  $('#closeDrawer').addEventListener('click',closeWord);els.scrim.addEventListener('click',closeWord);
...
function goToLesson(id){if(!id)return;location.hash=`lesson=${encodeURIComponent(id)}`;}

async function loadLesson(id){
  const meta=state.lessonIndex.find(item=>item.id===id)||state.lessonIndex[0];if(!meta)throw new Error('教材が見つかりません。');
  const res=await fetch(meta.path,{cache:'no-store'});if(!res.ok)throw new Error(`教材を読み込めませんでした (${res.status})`);
  const lesson=await res.json();state.lesson=lesson;state.selectedWord=null;state.drawerOpen=false;state.openSound=null;closeWord();
  renderLesson();renderMode();updateLessonNavigation();
...
function setMode(mode){state.mode=mode;state.openSound=null;$$('.mode-tabs [data-mode]').forEach(button=>{const active=button.dataset.mode===mode;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active));});if(mode!=='words')closeWord();renderMode();}
function renderMode(){
  if(!state.lesson)return;const lesson=state.lesson;els.translation.hidden=true;
  switch(state.mode){
    ...
    case'words':els.workspace.innerHTML=`<div class="word-list">${(lesson.words||[]).map((w,i)=>`<button class="word-chip" type="button" data-word-index="${i}" aria-expanded="${state.selectedWord===i&&state.drawerOpen}">${escapeHtml(w.surface)}</button>`).join('')}</div><p class="hint">単語を押すと発音し、詳細を開きます。別の語を押すと内容だけ安全に切り替わります。</p>`;break;
    ...
    case'sound':renderSoundLab();break;default:els.workspace.innerHTML='';
  }
}
...
function openWord(index){const word=state.lesson.words?.[index];if(!word)return;state.selectedWord=index;state.drawerOpen=true;speak(word.surface,.9);els.detailSurface.textContent=word.surface;const rows=[['IPA',word.ipa?`/${word.ipa}/`:null],['文脈上の意味',word.meaning_ja],['辞書形',word.lemma],['品詞',word.pos],['文中の働き',word.role],['形・活用',word.morphology],['短い解説',word.detail_ja]].filter(([,v])=>v);els.detailBody.innerHTML=rows.map(([label,value])=>`<div class="detail-row"><b>${escapeHtml(label)}</b><div>${escapeHtml(value)}</div></div>`).join('');els.drawer.classList.add('open');els.drawer.setAttribute('aria-hidden','false');els.scrim.hidden=false;updateWordAria();}
...
window.FSL_DEBUG={state,setMode,openWord,closeWord,toggleSound,loadLesson,resolveLessonId,moveLesson};
