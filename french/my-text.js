...
function initMyText(){
  ...
  list.addEventListener('click',(event)=>{
    ...
    const observeButton=event.target.closest('[data-mytext-observe]');
    if(observeButton){openObservation(observeButton.dataset.mytextObserve);return;}
  ...
  observe.addEventListener('click',(event)=>{
    ...
    if(token){selectObservedToken(Number(token.dataset.tokenIndex));return;}
    const saveNote=event.target.closest('[data-save-token-note]');
    if(saveNote)saveTokenNote(Number(saveNote.dataset.saveTokenNote));
  });
...
function saveCurrentText(){
  ...
  else{store.items.unshift({id:`my-${Date.now().toString(36)}`,text,created_at:new Date().toISOString(),updated_at:new Date().toISOString(),source:'user',confidence:'unverified',annotations:{}});if(store.items.length>100)store.items=store.items.slice(0,100);}
...
async function ensureLexicon(){
  ...
  try{const response=await fetch(LEXICON_PATH,{cache:'no-store'});if(!response.ok)throw new Error();const data=await response.json();const entries=Array.isArray(data.entries)?data.entries:[];lexicon=new Map(entries.map(entry=>[normalizeToken(entry.form),entry]));}
  catch{lexicon=new Map();setStatus('内蔵辞書を読み込めませんでした。My Text自体は引き続き利用できます。',true);}
  return lexicon;
}
...
async function openObservation(id){
  ...
  await ensureLexicon();
  const tokens=tokenizeFrench(item.text);
  activeObservation={itemId:id,tokens,selectedIndex:null};
  renderObservation();
}
...
function renderObservation(){
  const area=$('#myTextObserve');if(!area||!activeObservation)return;
  const item=store.items.find(x=>x.id===activeObservation.itemId);if(!item){closeObservation();return;}
  const tokens=activeObservation.tokens;
  const matched=tokens.filter(token=>lexicon?.has(normalizeToken(token))).length;
  area.hidden=false;
  area.innerHTML=`<div class="observe-head"><div><div class="eyebrow">MY TEXT OBSERVATION · UNVERIFIED</div><p lang="fr">${escapeHtml(item.text)}</p></div><small>${matched}/${tokens.length} 語が内蔵教材と照合</small></div><div class="token-row">${tokens.map((token,index)=>{const known=lexicon?.has(normalizeToken(token));return `<button type="button" class="token-chip ${known?'known':'unknown'}" data-token-index="${index}" aria-pressed="${activeObservation.selectedIndex===index}">${escapeHtml(token)}<small>${known?'既知':'未登録'}</small></button>`;}).join('')}</div>${renderTokenDetail(item)}`;
}

function renderTokenDetail(item){
  const index=activeObservation?.selectedIndex;
  if(index===null||index===undefined)return '<p class="hint">語を押すと、内蔵教材に確認済み情報がある場合だけ表示します。未登録語は推測しません。</p>';
  const token=activeObservation.tokens[index];
  const entry=lexicon?.get(normalizeToken(token));
  const note=item.annotations?.[annotationKey(index,token)]||'';
  const verified=entry?`<div class="token-detail-grid"><div><b>内蔵教材での形</b><span>${escapeHtml(entry.form)}</span></div><div><b>IPA</b><span>${entry.ipa?`/${escapeHtml(entry.ipa)}/`:'—'}</span></div><div><b>文脈例での意味</b><span>${escapeHtml(entry.meaning_ja||'—')}</span></div><div><b>辞書形</b><span>${escapeHtml(entry.lemma||'—')}</span></div><div><b>品詞</b><span>${escapeHtml(entry.pos||'—')}</span></div>${entry.detail_ja?`<div class="wide"><b>確認済み注記</b><span>${escapeHtml(entry.detail_ja)}</span></div>`:''}</div>`:`<div class="unknown-note"><b>未登録</b><p>この語について、現在の内蔵教材から安全に照合できる情報はありません。意味・品詞・発音を推測して補いません。</p></div>`;
  return `<section class="token-detail"><div class="sound-section-label">SELECTED WORD</div><h3 lang="fr">${escapeHtml(token)}</h3>${verified}<label class="token-note-label"><span>自分の注釈</span><textarea rows="2" data-token-note="${index}" placeholder="自分で気づいたことを記録">${escapeHtml(note)}</textarea></label><button type="button" class="history-clear" data-save-token-note="${index}">注釈を保存</button></section>`;
}

function selectObservedToken(index){if(!activeObservation||!activeObservation.tokens[index])return;activeObservation.selectedIndex=index;renderObservation();}
function saveTokenNote(index){
  if(!activeObservation||activeObservation.selectedIndex!==index)return;
  const item=store.items.find(x=>x.id===activeObservation.itemId);const token=activeObservation.tokens[index];const input=$(`[data-token-note="${index}"]`);if(!item||!token||!input)return;
  item.annotations=item.annotations&&typeof item.annotations==='object'?item.annotations:{};
  const key=annotationKey(index,token);const value=input.value.trim();if(value)item.annotations[key]=value;else delete item.annotations[key];item.updated_at=new Date().toISOString();persist();setStatus('注釈を端末内に保存しました。');renderObservation();
}

function tokenizeFrench(text){
  const matches=(text.match(/[\p{L}À-ÖØ-öø-ÿ]+(?:['’][\p{L}À-ÖØ-öø-ÿ]+)?/gu)||[]);const result=[];
  for(const raw of matches){const normalized=raw.replace(/’/g,"'");const apostrophe=normalized.indexOf("'");if(apostrophe>0&&apostrophe<normalized.length-1){result.push(normalized.slice(0,apostrophe+1));result.push(normalized.slice(apostrophe+1));}else result.push(normalized);}
  return result;
}
...
window.FSL_MYTEXT_DEBUG={tokenizeFrench,normalizeToken};
